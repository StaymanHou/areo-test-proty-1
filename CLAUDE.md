...
### Browser-walkthrough discipline

**Time-sensitive browser observations of physics / feel behavior MUST use the scripted-input harness (`?script=...`), NOT Playwright `dispatchEvent` keyboard simulation or `page.keyboard.press()`.** This rule binds all agent-side automated verify-self / verify-codify work going forward.

When a feature workflow's verify-self stage (or any agent-side automated probe) requires observing in-game flight behavior over a window > 2s, the agent MUST:

1. **Drive input via `?script=<spec>`** keyed on the fixed-timestep physics tick. Grammar: `hold:<KeyCode>@<startSec>:<endSec>` (e.g. `hold:ArrowUp@1.0:4.0`), `hold:Throttle=<float>@<startSec>:<endSec>` (e.g. `hold:Throttle=0.6@0:5.0`), comma-separated for multiple concurrent events, `:end` keyword for unbounded hold. Implemented in `src/engine/scripted-input.ts` + `src/engine/scripted-input-runner.ts`.
2. **Wait for completion** via `page.waitForFunction(() => window.__aircraft.isScriptComplete())`. NEVER `page.waitForTimeout(...)` — that's exactly the wall-clock dependency this harness eliminates.
3. **Read structured per-tick state** from `window.__aircraft.getScriptedLog()` (debug-only). Default `flight-feel` shape: `{tick, t_sec, position, linvel, rotation, angvel, pitch_deg, roll_deg, yaw_deg, AS_mps, alpha_deg, beta_deg, throttle}` per row. Do NOT screenshot-diff or DOM-scrape for physics state.
4. **Use `?config=<name>` for airframe-swap experiments** (e.g. `?config=aerobatic` loads `/config/aircraft-aerobatic.json`). Do NOT edit the production `aircraft.json` on disk for one-off agent probes. The config-name regex `/^[a-z0-9_-]+$/i` is the path-traversal defense at the boot path.
5. **Gate is `?debug=true` AND `?script=...`**. Without `?debug=true`, the harness silently warns and stays inactive (production build carries no overhead).

**Why:** Playwright keyboard events are subject to OS-level scheduling jitter that decouples observation from the fixed-timestep game loop. Origin: SURFACE-2026-06-06-04 — a Playwright probe read 107-150°/s roll rate vs a deterministic Vitest harness reading 179°/s at the same physics state during controls-feel-pass Phase 2 verify-self. The harness bypasses the DOM event layer entirely (synthesizes input state at the `onPhysics` tick boundary) and latches the log buffer once `isComplete()` first returns true (so the log is byte-stable across observation timing — the load-bearing determinism gate, codified in `tests/e2e/scripted-input.spec.ts`'s byte-identical-rerun assertion).

**Out of scope of this rule (still valid):**
- **Operator-as-feedback-reviewer manual keyboard walkthrough** for qualitative verify-human checks. This rule covers agent-side automated probes only.
