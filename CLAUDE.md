...
**Status (2026-05-23, post-WP14.11 ESCALATED + D18 architect cycle):** Phase 1 closed (WP1–WP9.6). Phase 2 progress: WP10/10.5/11/12/13/14 shipped 2026-05-12; D14 cascade landed (WP14.6 ✓, WP14.7 ✓, WP14.8 ✓, WP14.5-retry ✓). D17+D16 cascade mechanism halves shipped: **WP14.9b (D17 β4 non-dim pitch-rate damping) ✓ 2026-05-17 commit `0df9a07`**; **WP14.10 (D16 β5 non-dim AoA-rate damping) ✓ 2026-05-23 commit `27324aa`**. Joint-tune retry **WP14.11 ESCALATED 2026-05-23** (commit `9cbd3de`) — β4+β5 mechanism halves confirmed working numerically (multiple symmetric-mirror (clQ, clAlphaDot) points produce 1800 finite ticks across all 3 throttle regimes — a real partial-success vs WP14.5-retry's zero-finite-points box), but no point produces flyable trajectories (airspeed peaks 230-373 m/s, deployed-config scores ~-100M vs threshold -300). **D18 arch revision landed 2026-05-23** (`docs/product/arch.md` Revision 2026-05-23): third mechanism layer is the textbook drag polar — per-surface optional `inducedDragK?` (CL² induced-drag coupling, default 0) + top-level optional `fuselageDrag?: {cd0, area}` (body-level parasitic drag at body origin, default absent). Energy-balance arithmetic confirms the gap: current model gives V_term ≈ 245 m/s in unpowered dives vs observed 373 m/s peak — dissipation is undersized because the per-surface CD curve has no CL² coupling and no fuselage drag is computed (Rapier `linearDamping` is unset). Singular not stacked per `feedback_surface_or_means_or.md` — if D18 also escalates at WP14.12, the next candidate is D19 (inertia-tensor revision). **WBS additions:** **WP14.11.5 (D18 impl, size S–M, NEXT)** — schema extension + parse plumbing + computeAeroForce CD augmentation + flightmodel body-drag accumulator + triple-gate verify-self (Rules #1+#2+#4); NO `aircraft.json` change (deferred to WP14.12 per the D17/D16 two-close-gates precedent). **WP14.12 (8-dim joint tune, size S)** — `npm run tune` over (clQ, clAlphaDot, inducedDragK) × wings+h-stab + (fuselageDrag.cd0, fuselageDrag.area); scores deployed-symmetric airframe via `tools/tune/score-deployed.mjs` per `feedback_tune_cli_search_vs_deploy.md`; explicit browser-walkthrough verify-self gate closes the WP14.11-retrospect gap. Replaces ESCALATED WP14.11 as actionable joint-tune WP. Phase 2 mission content (WP15/WP16/WP17) remains paused — the pause line moves from "post-WP14" to "post-WP14.12 branch A". **Open SURFACE items:** **SURFACE-2026-05-23-01 (high — D18 cascade driver; closes-by-implementation at WP14.12 branch A; supersedes -16-04 as actionable arch-revision driver)**; SURFACE-2026-05-17-03 (high — partial close at WP14.11, narrowing was right call; transitively closes at WP14.12 branch A); SURFACE-2026-05-17-01 + SURFACE-2026-05-16-01 + SURFACE-2026-05-16-04 + SURFACE-2026-05-12-03 + SURFACE-2026-05-12-01 + SURFACE-2026-05-11-04 (all partial / blocked-by -23-01; transitively close at WP14.12 branch A); SURFACE-2026-05-17-02 (medium — arch.md D17 cross-product order errata, non-blocking); SURFACE-2026-05-11-02 (medium); SURFACE-2026-05-16-03 (low — arch.md doc typo); SURFACE-2026-05-16-02 (low — perf flake); SURFACE-2026-05-12-02 (low — test-mission pollution); SURFACE-2026-04-19-01 (Phase 3 — bundle).
