...
**Status (2026-05-24, post-`tune-cli-link-flag` + D20 architect cycle):** Phase 1 closed (WP1–WP9.6). Phase 2 progress: WP10/10.5/11/12/13/14 shipped 2026-05-12; D14 cascade landed (WP14.6/7/8 + WP14.5-retry ✓). D17+D16+D18+D19 cascade mechanism layers all shipped: **WP14.9b (D17 β4 non-dim pitch-rate damping) ✓ 2026-05-17 commit `0df9a07`**; **WP14.10 (D16 β5 non-dim AoA-rate damping) ✓ 2026-05-23 commit `27324aa`**; **WP14.11.5 (D18 drag polar — induced + fuselage parasitic) ✓ 2026-05-23 commit `a93c277`**; **WP14.13 (D19 widened-bounds tune) ESCALATED 2026-05-24 commit `e5933f4`** — search-vs-deploy ratio INVERTED to 0.67× revealed asymmetric/symmetric methodology mismatch as binding. Tooling fix: **`tune-cli-link-flag` task ✓ 2026-05-24 commit `58badfe`** — added `--link <src>=<dst>` flag to `tools/tune/tune.ts` (ratio collapsed to 1.0×, deployed score improved ~14× over WP14.12 + ~447× over WP14.13 in 15s linked smoke). **D20 arch revision landed 2026-05-24 PM** (`docs/product/arch.md` Revision 2026-05-24 PM): re-tune drag polar under `--link surfaces.0=surfaces.1` with further-widened bounds (`inducedDragK_{wing,hstab}` 1.5→3.0; `fuselageDrag.area` 10.0→20.0; `surfaces.2.clQ` 3.0→4.0). Two clear "bound-is-a-floor" signals at linked smoke survived methodology fix: k_wing=86%-of-D19-bound, area=79%-of-D19-bound — direct optimizer-at-bounds per `feedback_optimizer_bounds_are_floor.md`. Per CLAUDE.md Rule #5 architect-cycle derivation through the artifact-optima lens (per `feedback_tune_cli_search_vs_deploy.md` Quaternary use): D20 picks further bound-widening singularly over D19d (envelope re-cal — reserved as Branch-B candidate) / D19b (Ixx-anchored inertia — still rejected as causally weak) / fifth-mechanism (defer) / option-c (accept current state, ~−6.7M deployed, ship as Phase 2 starting point — reserved at next cycle if D20 refutes too). **WBS addition:** **WP14.14 (D20 linked-search re-tune, size S, NEXT)** — `npm run tune -- --link surfaces.0.{clQ,clAlphaDot,inducedDragK}=surfaces.1.* --knobs ... --bounds 0..3,0..10,0..3.0,0..4,0..10,0..3.0,0..1.0,0..20.0 --seed 42 --restarts 4`; criteria 1+2+browser; full close of SURFACE-24-03 + 8 transitively-blocked SURFACEs on Branch A. Phase 2 mission content (WP15/WP16/WP17) remains paused — pause line moves from "post-WP14.13 branch A" to "post-WP14.14 branch A". **Open SURFACE items:** **SURFACE-2026-05-24-03 (high — D20 cascade driver; partial-close at `tune-cli-link-flag` methodology fix; full close at WP14.14 Branch A)**; SURFACE-24-01 + SURFACE-23-01 + the chain of 7 transitively-blocked (SURFACE-17-03, -17-01, -16-04, -16-01, -12-03, -12-01, -11-04) all resolved-by-cascade-but-partial; transitively close at WP14.14 Branch A; SURFACE-24-02 (low — axis-naming docs errata); SURFACE-17-02 (medium — arch.md D17 cross-product order errata, non-blocking); SURFACE-11-02 (medium); SURFACE-16-03 / SURFACE-16-02 / SURFACE-12-02 (low); SURFACE-04-19-01 (Phase 3 — bundle).
