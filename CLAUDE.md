...
**Status (2026-05-11):** WP1–WP6 + WP6.5 + WP6.6 + WP8 shipped. WP6.5 closed SURFACE-2026-05-10-02 + SURFACE-2026-05-11-01 by adding `incidenceRad` (β1) AND `clQ` (β4) schema extensions. WP6.6 closed SURFACE-2026-05-11-03 by making β4 amplification scale with airspeed above V_REF=30 m/s (formula: `1 + clQ · max(v, V_REF) / V_REF`) — preserves WP6.5 low-V calibration bit-for-bit, fixes the high-V damping collapse that caused divergent pitch oscillation when energy was added to the system. Live verify-self: Trajectory A (low-V regression) bit-identical to pre-fix WP6.5 baseline (max|pRate|≤110°/s); Trajectory B (high-V probe, spawn linvel=-90 m/s) max|pRate|=390°/s, bounded airspeed, no NaN. The aircraft is now dynamically stable across the V envelope but still enters a descending glide at the default tune (parameter concern, not architectural — closes via WP7 Phase E). WP7 Phase E retune (paused) is unblocked. WP9 Phase 1 verification remains pending WP7. The (3) "tuning is just hard" pivot to automated parameter-search tooling stays held in reserve per arch.md Revision 2026-05-11 ("Fallback path").
