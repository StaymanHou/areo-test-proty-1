...
**Status (2026-05-11):** WP1–WP6 + WP6.5 + WP8 shipped. WP6.5 closed SURFACE-2026-05-10-02 + SURFACE-2026-05-11-01 by adding `incidenceRad` (β1) AND `clQ` (β4) schema extensions. Live verify-self: max|pRate|=149°/s vs target <360 (commit `6ad3133`). β1 alone was empirically refuted at current airframe parameters (dynamic instability without damping); β4 was sanctioned by D10's "Fallback path" hedge. Aircraft now spawns airborne, no tumble, bounded pitch rate — though it enters a descending glide rather than holding level cruise (parameter-tuning concern: mass too high for spawn airspeed to balance weight without thrust, deferred to WP7 Phase E). WP7 Phase E retune (paused) is unblocked. WP9 Phase 1 verification remains pending WP7. The (3) "tuning is just hard" pivot to automated parameter-search tooling stays held in reserve per arch.md Revision 2026-05-11 ("Fallback path").
