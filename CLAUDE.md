...
**Status (2026-05-11):** WP1–WP8 all shipped (WP1, WP2, WP3, WP4, WP5, WP6, WP6.5, WP6.6, WP7, WP8). **Only WP9 (Phase 1 verification) remains for Phase 1 completion.** WP7's tuning pass empirically refuted single-knob improvements over the WP6.5 baseline — the phugoid mode is undamped at the Phase 1 airframe and produces divergent oscillation under any non-zero baseline throttle. Phase E shipped option (c) (accept the WP6.5 baseline = descending-glide attractor). Phase F's feel-check accepted at the "bounded, controllable, non-tumbling" bar via operator-as-tester. The undamped phugoid is logged as SURFACE-2026-05-11-04 (Phase 2 candidate). Earlier session: WP6.5 added `incidenceRad` (β1) + `clQ` (β4) schema extensions to give the airframe a level-trim equilibrium; WP6.6 made β4 airspeed-scaled (formula `1 + clQ · max(v, V_REF) / V_REF` with V_REF=30 m/s) to keep damping ratio constant across the V envelope. WP9 should evaluate cross-browser FPS and conduct the actual "developer takes off, flies, crashes" exit-criteria check; if the external feedback rejects the descending glide, escalate to Phase 2 phugoid-damping work (SURFACE-2026-05-11-04).
