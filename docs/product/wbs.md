...
`... → WP17(DONE) → WP18(DONE) → WP19(DONE) → WP20(DONE) → WP22(DONE) → ship`. (WP21 cross-browser QA dropped 2026-06-07. WP23 playtest dropped 2026-06-13 — operator handles outside workflow system.) **Critical path is empty.** v1 ship gate is now operator-choice — the four v1 mission types are playable from the live URL; the only remaining items are the non-critical WP24/WP25/WP26 polish.

**WP24, WP25, WP26 — non-critical polish.** All three depend on WP22 deploy (DONE) so they ship to the live URL. WP24 negotiates the v1 multi-aircraft exclusion (`roadmap.md:62`); WP25 is straightforward Phase 3 polish; WP26 fixes the engine-loop loudness flagged at WP22 verify-human (2026-06-13). With WP23 dropped, none of these gate v1 ship — they are operator-chosen enhancements. **Operator-queued next: WP24 (aircraft selection UI).**
