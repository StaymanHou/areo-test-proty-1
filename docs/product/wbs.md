...
### WP23: Playtesting — DROPPED (2026-06-13, operator directive)
**Status:** Dropped from v1 scope. Operator will handle playtesting outside the workflow system. Same precedent as WP21 (Chromium-only cross-browser drop). If any playtest feedback surfaces a SURFACE-worthy issue, file it manually.

### WP24: Aircraft selection UI ✅ SHIPPED 2026-06-13 (commit f3daffb)
**Description:** Surfaces airframe choice on the mission-select screen so players can pick before launching a mission. Picker shows "Trainer (Cessna)" (default) + "Jet (MiG-15)" segmented buttons above the mission list; persists to localStorage `flightsim.aircraft.selected`; mission tiles label their pinned-config overrides inline (Combat → "Combat [MiG-15]"). Boot-time resolution precedence is URL `?config=` > preloaded-mission `config?` > localStorage > default. Mid-session airframe change reloads via `?mission=<id>` deep-link so the boot path honors the new pick (per WP14.20 Phase A "mid-session swap unsupported" constraint). Aerobatic stays a test fixture per SURFACE-2026-06-06-06 Phase B deferred.
**Phase:** 3
**Dependencies:** WP22
**Size:** S–M (shipped at S — actual implementation kept narrow: one new module + select.ts extension + 5-line main.ts boot wiring + 4-line onSelect reload branch)
**Vision-constraint resolution:** `docs/product/roadmap.md:62` "Multiple aircraft selection" exclusion **negotiated into v1** at /feature-spec — cost calculus shifted since WP14.21 jet already exists; only the UI + per-mission label was missing. Roadmap text updated at finalize.
**Spec/plan resolutions (all 5 open questions accepted from /feature-spec defaults):**
- Free across all 4 missions; per-mission `config?` is a hard override (Combat keeps forced-MiG-15).
- Aerobatic stays test fixture, not in picker (defer to Phase B feel-tune).
- Labels read "Class (Airframe)" — "Trainer (Cessna)" / "Jet (MiG-15)".
...
### WP26: Audio mix pass — quieter engine + volume control ✅ SHIPPED 2026-06-13 (commit `9ce3644`)
...
**Size:** S
**Decisions landed at ship:**
- Single master slider — covers entire mix via existing `AudioEngine` master gain node.
- Slider lives on mission-select chrome alongside the aircraft picker (same DOM-overlay pattern).
- Persistence under `flightsim.volume.master` (0..1 float); default 0.5 on fresh storage; clamp [0,1], reject NaN/Infinity, swallow storage exceptions.
- Engine baseline `MAX_GAIN` lowered 0.2 → 0.12 (40% drop) so at-master=1.0 is the new "loud".
**Verification:** Vitest 854/854 (+8 select.test.ts + 15 new master-volume.test.ts + 3 audio-engine WP26 cases = +26 new); Playwright e2e 55/56 (3 new master-volume.spec.ts PASS; the 1 unrelated fail is pre-existing SURFACE-2026-06-13-PHUGOID-HIGH-REGRESSION); tsc + production build clean.

---

## Critical path

`... → WP17(DONE) → WP18(DONE) → WP19(DONE) → WP20(DONE) → WP22(DONE) → WP24(DONE) → WP26(DONE) → ship`. (WP21 cross-browser QA dropped 2026-06-07. WP23 playtest dropped 2026-06-13. WP25 control manual dropped 2026-06-13 — redundant with WP18 key-hints overlay.) **Critical path is empty; all live WBS items shipped.** v1 ship gate is now operator-choice — every v1 mission type is playable from the live URL, both airframes selectable, audio mixed to player taste with persistence.
