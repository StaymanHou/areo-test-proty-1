...
stage: wbs
state: in-progress
updated: 2026-05-17 (D17 arch revision landed via /product-arch — supersedes D15 with textbook non-dim pitch-rate-damping form parallel to D16; WP14.9 stays ESCALATED in audit trail; new WP14.9b added for D17 implementation; WP14.10 dependency updated from WP14.9 → WP14.9b; WP14.11 bounds + dependencies updated to dimensionless [0..15] per surface and depend on WP14.9b. Phase 2 mission content (WP15/WP16/WP17) remains paused at post-WP14 line until WP14.11 closes.)
---
...
### WP14.9b: β4 non-dimensional pitch-rate damping (D17)
...
**Tasks:**
- [ ] Plan-time: confirm the proposed CL-augmentation site (step 4b in `computeAeroForce`, alongside β5 D16) is the right layer per arch.md D17. The site is unambiguous in D17 prose; this task is a sanity check before coding.
- [ ] Add `dampAxis: Vector3` per-surface cached field on `AeroSurface`. Computed at construction as `(position × restNormal).normalized()` BEFORE incidence-rotation (matches `spanAxis` derivation timing). Refreshed by `setGeometry({position?, normal?, chord?})` whenever position OR (normal-via-restNormal) changes — analogous to the existing `spanAxis` refresh.
- [ ] Add a module-scoped `_scratchDampAxisWorld: Vector3` buffer (parallel to existing scratch buffers) for the per-tick world-frame projection via `_scratchDampAxisWorld.copy(surface.dampAxis).applyQuaternion(bodyState.quaternion)`. Allocation-free hot path preserved.
- [ ] **Code change:** in `computeAeroForce`, remove the WP6.6 amplification at lines 446-450 (the `if (surface.clQ !== 0) { ... multiplyScalar(1 + surface.clQ * vScale); }` block). The `ω × r` cross product stays — it's the linear airflow contribution and is unaffected by D17. Add a new branch at step 4b (alongside β5 D16, BEFORE the `surface.prevAoA = alpha` cache update so β4 and β5 augmentations compose cleanly):
  ```typescript
  if (surface.clQ !== 0) {
...
- [ ] Vitest: dampAxis derivation for all 4 current aircraft.json surfaces. Wings: `(±2, 0, 0) × (0, 1, 0) = (0, 0, ∓2)` → normalized `(0, 0, ∓1)` = roll axis. H-stab: `(0, 0, 3) × (0, 1, 0) = (-3, 0, 0)` → normalized `(-1, 0, 0)` = pitch axis. V-stab: `(0, 0.5, 3) × (1, 0, 0) = (0, 3, -0.5)` → normalized `(0, 0.987, -0.164)` = primarily yaw axis. Codify these as a parameterized test over the existing 4 surface configurations.
- [ ] Vitest: closed-form non-dim sanity. At `clQ=1, ω_pitch=1 rad/s, chordLength=1, vBody=30, dampAxis aligned with ω` → CL augmentation = `1 · 1 · 1 / 60 ≈ 0.0167`. At `vBody=15` (below V_REF) → `1 · 1 · 1 / 60 ≈ 0.0167` (floor active). At `vBody=60` (above V_REF) → `1 · 1 · 1 / 120 ≈ 0.0083` (linear decrease with V, opposite of the cubic growth the old form gave).
- [ ] Vitest: clQ=0 control parity. Existing tests that exercise `clQ=0` (v-stab in aircraft.json baseline; multiple unit-test fixtures) MUST continue to produce bit-identical force vectors. This is the asymmetric-fix-no-op contract per `feedback_asymmetric_fix_no_op.md` — the fix is a no-op when the mechanism is disabled. 516/516 Vitest at clQ=0 baseline is binding.
- [ ] Vitest: re-baseline the existing `clQ amplification grows with airspeed above V_REF` test at `aerosurface.test.ts:916` — under D17 the assertion changes shape. The new contract: at non-zero clQ, CL augmentation magnitude DECREASES with V (factor `c̄ / (2V)`), but the resulting DAMPING FORCE (which scales with V² via dynamic pressure) still grows linearly with V. Rewrite the assertion to test the linear-V damping-force growth rather than the cubic-V CL-amplification growth. Sign convention per arch.md D17 — positive clQ on h-stab produces negative CL augmentation for positive pitch rate (nose-up damping).
- [ ] Harness-vs-browser parity test `tests/parity-diff.test.ts`: under D17 the parity-CSV regenerates. Run `npm run test:e2e -- tests/e2e/parity.spec.ts` to regenerate browser CSVs; verify Vitest parity-diff stays green (bit-identity between browser and Node-harness within `|Δ| < 1e-6`). The reference trajectories change shape (D17 changes behavior at all V, not just above V_REF — per arch.md Risk 1); bit-identity within the new shape is preserved.
- [ ] **verify-self via harness (BINDING — CLAUDE.md Rule #2 + Rule #4):** three harness runs at the `throttle-high` fixture (1800 ticks):
  1. **Baseline** (post-WP14.11-tuned `clQ` values, or pre-tune `clQ=3,8` if WP14.11 hasn't run yet at WP14.9b verify-self time — in which case the baseline run is informational, not a gate). Must stay finite under the post-tune values.
  2. **Non-default** (e.g. `clQ=12` on all surfaces, or the optimizer's recommended high-stress point). Must stay finite. Rule #2 gate.
  3. **Control `clQ=0`** on all surfaces. Must stay finite — Rule #4 gate per CLAUDE.md `### Physics-mechanism discipline` (added 2026-05-17). Confirms β4 is the *active* augmentation and not a hidden interaction.
- [ ] **verify-self via harness (BINDING — CLAUDE.md Rule #1 — live observation at non-zero coefficient in ≥2 regimes):** Playwright probe via `tests/e2e/parity.spec.ts` (already Playwright-driven, satisfies Rule #1 source) at non-zero clQ in low-throttle (0.05) AND high-throttle (0.40) for ≥10s each. Confirms sign convention is physically right before the regression test at `aerosurface.test.ts` is rewritten.
- [ ] **Coordinate with WP14.11:** the close gate for WP14.9b is "code lands + parity-CSV regenerates + harness verify-self triple passes." The Phase-2-mission-blocking gate (does the airframe actually fly stable?) is WP14.11's job, NOT WP14.9b's. WP14.9b can close with default `clQ=3,8` even if those values turn out unstable post-D17, as long as the IMPLEMENTATION is sound. WP14.11 is the gate that proves the *airframe* is stable. **Two close gates, two WPs** — the WP14.9 escalation taught the lesson that conflating these creates ambiguity.
- [ ] Update SURFACE-2026-05-17-01 status in `workflow/backlog.md` — resolved-by-implementation when WP14.9b closes + WP14.11 finds a stable point. SURFACE-2026-05-16-01 closes at the same gate. SURFACE-2026-05-16-04 β4 side closes at the same gate.
...
**Phase:** 2
**Dependencies:** WP14.9 (cached `chordLength` field — landed in production via the WP14.9 escalation commit `9ebaf47` and retained; D16 consumes it identically to D17/WP14.9b). Operationally WP14.10 and WP14.9b are mechanically independent (different rate-derivatives of CL) but tend to land together to enable the WP14.11 joint tune.
**Size:** S
**Tasks:**
