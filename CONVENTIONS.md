...
`AeroSurface.inducedDragK` is the **induced-drag coefficient** (D18, textbook drag-polar `k = 1 / (π · AR · e)`). When set, the drag coefficient is augmented by `inducedDragK · cl²` at step 4d of `computeAeroForce`, applied AFTER β4 and β5 CL augmentations so the **post-augmentation `cl`** drives the induced-drag rise (textbook lifting-line theory: `CD_i = CL_actual² / (π · AR · e)`, where the unsteady contributions are part of the total circulation-bound lift). **Sign: `inducedDragK ≥ 0`** — drag opposes motion, so the augmentation is always non-negative; `parseAircraftConfig` rejects negative values with a physical-rationale error. The augmentation is sign-symmetric in `cl` (cl² is positive in both signs of cl), correctly modeling that induced drag is present in both positive- and negative-G flight. Default `inducedDragK = 0` → no augmentation → identical behavior to pre-D18. Textbook starting values (per the airframe's planform): wings `k ≈ 0.15` (AR ≈ 2.7, e ≈ 0.8); h-stab `k ≈ 0.25` (smaller AR). Tuning is WP14.12's joint-tune problem. See `arch.md` Revision 2026-05-23 (D18) and `SURFACE-2026-05-23-01`.

`AircraftConfig.fuselageDrag` is the **body-level parasitic drag** for the non-lifting fuselage (D18). When present, a single drag force of magnitude `0.5 · ρ · V² · area · cd0` is applied at the **body origin** opposite to the linear velocity vector (`F = −0.5·ρ·V²·area·cd0 · (linvel/|linvel|)`). Applied at the body origin via `addForce` (NOT `addForceAtPoint`), so the contribution is purely translational with zero torque — fuselage drag is not a moment-arm mechanism. **Sign: both `cd0 ≥ 0` and `area ≥ 0`** — drag is dissipative; `parseAircraftConfig` rejects negative values. A `V > 1e-6` guard skips the augmentation at body rest to avoid 0/0. Default-absent (`fuselageDrag` omitted from `aircraft.json`) → no body-level drag → identical behavior to pre-D18. Textbook starting values for a Cessna-class airframe: `cd0 ≈ 0.3`, `area ≈ 1.5 m²` (fuselage drag-area ≈ 0.45 m²). See `arch.md` Revision 2026-05-23 (D18) and `SURFACE-2026-05-23-01`.
...
After WP14.7 Phase 3, `physics-core/` has exactly two production callers: one browser caller (`src/main.ts` via the game loop) and one Node caller (`tools/tune/harness.ts`). Both must produce identical trajectories for the same fixture under the same parameters — that's the load-bearing acceptance contract for the WP14.8 Nelder-Mead optimizer (its objective function evaluates `tools/tune/harness.ts` and compares to a reference trajectory; optimizer convergence depends on harness determinism and on harness↔browser parity, so a player tuning live in the browser experiences the same physics the optimizer was fitted against).

The boundary is enforced by the `tests/parity-diff.test.ts` parity test, which uses a precedence: prefer `test-results/harness-trajectory-<id>.csv` (produced by `npm run harness:parity`) for the Node side when available, falling back to an in-process Vitest synthetic stub when only the browser CSV is present. Both runners must remain bit-identical to `|Δ|<1e-6` on all finite trajectory rows; non-finite rows must match in *kind* (NaN-vs-NaN, +Infinity-vs-+Infinity, -Infinity-vs--Infinity) — the parity-of-divergence semantics added in WP14.7 Phase 1.7 to accommodate the SURFACE-2026-05-16-01 β4 instability that the throttle-high fixture exercises by design. The fallback synthetic-stub path preserves the "Vitest-alone is a smoke for everything except parity" contract WP14.6 established. A real parity drift between the two surfaces surfaces a real bug — including the `Aircraft.reset()` force-accumulator bug that the parity test caught during WP14.6 build.
